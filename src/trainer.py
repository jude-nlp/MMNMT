...
from .utils import to_cuda, concat_batches, find_modules, truncate
...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        if params.stopping_criterion != '':
            ...
            assert len(split) == 2 and split[1].isdigit()   # for example: 'valid_en-fr_mt_bleu,10'
    ...
    def set_parameters(self):   # update 
        """
        Set parameters.
        """
        params = self.params
        ...
        # encoder   update 用于设置编码器优化器，这里暂时用不到了 
    ...
    def print_avg_sim(self):
        sim0 = torch.Tensor(self.layer_sim['layer_0']).mean()
        sim1 = torch.Tensor(self.layer_sim['layer_1']).mean()
        sim2 = torch.Tensor(self.layer_sim['layer_2']).mean()
        sim3 = torch.Tensor(self.layer_sim['layer_3']).mean()
        sim4 = torch.Tensor(self.layer_sim['layer_4']).mean()
        sim5 = torch.Tensor(self.layer_sim['layer_5']).mean()
        logger.info('sim0:%.4f' % sim0)
        logger.info('sim1:%.4f' % sim1)
        logger.info('sim2:%.4f' % sim2)
        logger.info('sim3:%.4f' % sim3)
        logger.info('sim4:%.4f' % sim4)
        logger.info('sim5:%.4f' % sim5)
    ...
    def get_disc_iterator(self, iter_name, lang1, lang2):  # Update 
        """
        Build data iterator.
        """
        logger.info("Creating new discriminator data iterator (%s) ..." )
        iterator = self.data['disc']['train']['x'].get_iterator(
            shuffle=True,
            group_by_size=self.params.group_by_size,
            return_indices=True,
        )
    ...
    def get_batch(self, iter_name, lang1, lang2=None, stream=False):
        ...
        if iter_name != 'disc': # 如果不是判别器
            ...
            iterator = self.iterators.get((iter_name, lang1, lang2), None)
            if iterator is None:
                iterator = self.get_disc_iterator(iter_name, lang1, lang2)
            try:
                x = next(iterator)
            except StopIteration:
                iterator = self.get_disc_iterator(iter_name, lang1, lang2)
    ...
    def mlm_step_disc(self, lang1, lang2, lambda_coeff):
        """
        ...
        mlm_loss = lambda_coeff * loss

        """ disc_step """
        """ ------- """
        # get batch
        (x, lengths), idx = self.get_batch('disc', None, None)
        x, lengths = truncate(x, lengths, params.max_len, params.eos_index)
        y = self.data['disc']['train']['y'][idx]

        # cuda
        x, y, lengths = to_cuda(x, y, lengths)

        # loss
        h = self.encoder('fwd', x=x, lengths=lengths, positions=None, langs=None, causal=False)[0]
        # alpha = 2. / (1. + np.exp(-1 * self.epoch)) - 1
        alpha = 0.25  
        reverse_h = ReverseLayerF.apply(h, alpha)   # update 
        output = self.proj(reverse_h)
        disc_loss = F.cross_entropy(output, y, weight=None) + 0 * sum(p.sum() for p in self.encoder.parameters())
        self.stats['DISC'].append(disc_loss.item())

        """ optimize """
        """ ------- """
        loss = mlm_loss + disc_loss
    ...
    def align_step(self, lang1, lang2):
        """
        Align different languages into same representations.
        """
        params = self.params
        name = 'model' if params.encoder_only else 'encoder'
        model = getattr(self, name)
        ...
        if params.get_layer_tensors:
            layer_tensors1, enc1 = model('fwd', x=x1, lengths=len1, langs=langs1, causal=False)
            layer_tensors2, enc2 = model('fwd', x=x2, lengths=len2, langs=langs2, causal=False)
            for i, (tensor1, tensor2) in enumerate(zip(layer_tensors1, layer_tensors2)):
                tensor1 = tensor1.transpose(0, 1)
                tensor2 = tensor2.transpose(0, 1)
                # logger.info('src size %s' % str(tensor1.size()))
                # logger.info('tgt size %s' % str(tensor2.size()))
                max_pool1 = torch.nn.MaxPool2d((len(tensor1[0]),1), stride=1)
                max_pool2 = torch.nn.MaxPool2d((len(tensor2[0]),1), stride=1)
                tensor1 = max_pool1(tensor1).squeeze() # torch.Size([16, 1, 512]) -> torch.Size([16, 512])
                tensor2 = max_pool2(tensor2).squeeze()
                feature1 = F.normalize(tensor1)
                feature2 = F.normalize(tensor2)
                sim = feature1.mm(feature2.t())
                avg_dis = 1 - torch.trace(sim) / len(tensor1)
                # logger.info('sim size: %s' % str(sim.size()))
                # logger.info('len(tensor1): %s' % str(len(tensor1)))
                self.layer_sim[('layer_%d' % i)].append(avg_dis)
                self.n_sentences += params.batch_size
        else:      
            # encode sentence lang1/lang2
            enc1 = model('fwd', x=x1, lengths=len1, langs=langs1, causal=False)    # torch.Size([37, 38, 512])  [句子长度，batch_size，dim]
            enc1 = enc1.transpose(0, 1) # torch.Size([38, 37, 512]) [batch_size，句子长度，dim]

            enc2 = model('fwd', x=x2, lengths=len2, langs=langs2, causal=False)
            ...
            enc1 = max_pool1(enc1).squeeze() # torch.Size([16, 1, 512]) -> torch.Size([16, 512])
            enc2 = max_pool2(enc2).squeeze()
            # logger.info('enc size: %s' % str(enc1.size()))
            # calculate cos similarity
            feature1 = F.normalize(enc1)
            feature2 = F.normalize(enc2)
            sim = feature1.mm(feature2.t())
            # logger.info('sim size: %s' % str(sim.size()))

            # loss
            loss = 1 - torch.trace(sim) / len(enc1)
    ...
    def draw_ellipse(self, position, covariance, ax=None, **kwargs):
        ...
        if covariance.shape == (2, 2):
            U, s, Vt = np.linalg.svd(covariance)# 协方差矩阵进行奇异值分解
    ...
    def plot_gmm(self, gmm, X, label=True, ax=None):
        ...
        ax.axis('equal') # x y 轴坐标刻度一致
        
        w_factor = 0.2 / gmm.weights_.max()
        for pos, covar, w in zip(gmm.means_, gmm.covariances_  , gmm.weights_):
            self.draw_ellipse(pos, covar, alpha=w * w_factor)
        plt.savefig('./src/gmm.png')

    def gmm_step(self, lang1, lang2):
        """
        PCA
        GMM cluster.
        """
        ...
        # 遍历所有数据
        iterator1 = self.data['mono'][lang1]['train'].get_iterator(
                    shuffle=True,
                    group_by_size=self.params.group_by_size,
                    n_sentences=-1,
                )
        iterator2 = self.data['mono'][lang2]['train'].get_iterator(
                    shuffle=True,
                    group_by_size=self.params.group_by_size,
                    n_sentences=-1,
                )
        ...
        for item in iterator1:
            ...
            # average pooling
            avg_pool1 = torch.nn.AvgPool2d((len(enc1[0]),1), stride=1)
            enc1 = avg_pool1(enc1).squeeze()
            enc1 = enc1.cpu()
            enc1 = enc1.detach().numpy().tolist()
            features1.extend(enc1)
        ...
        for item in iterator2:
            ...
            # average pooling
            avg_pool2 = torch.nn.AvgPool2d((len(enc2[0]),1), stride=1)
            enc2 = avg_pool2(enc2).squeeze()
            enc2 = enc2.cpu()
            enc2 = enc2.detach().numpy().tolist()
            features2.extend(enc2)
        ...
        pca1.fit(features1)
        features1 = pca1.transform(features1)

        pca2 = PCA(n_components=2)
        pca2.fit(features2)
        features2 = pca2.transform(features2)
        ...
        features1 = np.array(features1)
        logger.info(np.array(features1).shape)
        np.random.shuffle(features1)

        rng = np.random.RandomState(13) # random number generation
        features1_stretched = np.dot(features1, rng.randn(2, 2))
        gmm = GMM(n_components=4, covariance_type='full', random_state=42)
        self.plot_gmm(gmm, features1_stretched)

        '''
        # GMM 聚类
        gmm = GMM(n_components=2).fit(features1)
        labels = gmm.predict(features1)
        prob = gmm.predict_proba(features1)

        # 散点图
        logger.info(features1[:10])
        logger.info(features1[:, 0][:10])
        plt.scatter(features1[:, 0], features1[:, 1], c=labels, s=prob.max(1), cmap='viridis')
        plt.savefig('./src/gmm.png')
        # logger.info(np.array(features1).shape)
        # logger.info(np.array(features2).shape)
        logger.info(labels[:10])
        # logger.info(sum(pca2.explained_variance_ratio_))
        '''
        sys.exit()
        


class ReverseLayerF(torch.autograd.Function):   # Update
...
class SingleTrainer(Trainer):
    ...
    def disc_step(self):    # update
        ...
        # loss
        h = self.model('fwd', x=x, lengths=lengths, positions=None, langs=None, causal=False)[0]
        output = self.proj(h)
        loss = F.cross_entropy(output, y, weight=None) + 0 * sum(p.sum() for p in self.model.parameters())
        ...
        # append stats
        self.stats['DISC'].append(loss.item())
...
class EncDecTrainer(Trainer):
    ...
    def disc_step(self):    # update
        ...
        output = self.proj(h)
        loss = F.cross_entropy(output, y, weight=None) + 0 * sum(p.sum() for p in self.encoder.parameters())

        alpha = 2. / (1. + np.exp(-1 * (self.epoch - 58))) - 1
        loss = loss * alpha
    ...
    def mt_step_dann(self, lang1, lang2, lambda_coeff):
        """
        dann step.(Domain-Adversarial Training of Neural Networks)
        Machine Translation model and language classifier.
        """
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        ...
        self.stats[('AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(mt_loss.item())
        # loss = lambda_coeff * loss + 0 * sum(p.sum() for p in self.encoder.parameters())
        mt_loss = lambda_coeff * mt_loss
        ...
        alpha = (2. / (1. + np.exp(-1 * (self.epoch))) - 1) / 2 # 最大1/2
        reverse_h = ReverseLayerF.apply(h, alpha)   # update
        # reverse_h = h 
        output = self.proj(reverse_h)
        disc_loss = F.cross_entropy(output, y, weight=None) + 0 * sum(p.sum() for p in self.encoder.parameters())
        ...
        # number of processed sentences / words
        self.n_sentences += params.batch_size + 28 # update batch_size=4的话，一个epoch跑的时间太长了
        self.stats['processed_s'] += len2.size(0)
        self.stats['processed_w'] += (len2 - 1).sum().item()

    def disc_step_dann(self, lambda_coeff):    # lang_discriminator
        ...
        y = self.data['disc']['train']['y'][idx]
        bs = len(lengths)
        # logger.info("disc: %d sents per batch." % bs)   # update 
        # cuda
        x, y, lengths = to_cuda(x, y, lengths)
...
class XlmEncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        """
        Machine translation step with double encoder(xlm_enc + normal encoder).
        """
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        ...
        # xlm_enc
        enc0 = self.xlm_enc('fwd', x=x1, lengths=len1, langs=langs1, causal=False)
        enc0 = enc0.transpose(0, 1)

        # encode source sentence
        enc1 = self.encoder('fwd', x=x1, xlm_output=enc0, lengths=len1, langs=langs1, causal=False)
        enc1 = enc1.transpose(0, 1)

...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if lang in params.ae_steps or lang in params.bt_src_langs or len(params.gmm_steps) != 0:
                
                logger.info("loading dnone stream data")
...
def load_para_data(params, data):
    ...
    # required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps + params.align_steps) # update 8-14
    # 再加上 params.eval_mt_steps
    required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps + params.align_steps + params.eval_mt_steps) # update 8-17
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if splt != 'train':
                ...
                dataset.remove_empty_sentences()                # updata
                # dataset.remove_long_sentences(params.max_len)
...
def load_disc_data(params, data):   # update load辨别器数据
    data['disc'] = {splt: {} for splt in ['train', 'valid', 'test']}
    dpath = os.path.join(params.data_path, 'discriminator')
    for splt in ['train', 'valid', 'test']:

        # load data and dictionary
        data1 = load_binarized(os.path.join(dpath, '%s.s1.pth' % splt), params)
        set_dico_parameters(params, data, data1['dico'])
        data['disc'][splt]['x'] = Dataset(data1['sentences'], data1['positions'], params)
        ...
        if splt != 'test':
            ...
            with open(os.path.join(dpath, '%s.label' % splt), 'r') as f:
                ...
                data['disc'][splt]['y'] = torch.LongTensor(y)
                assert len(data['disc'][splt]['x']) == len(data['disc'][splt]['y'])
        logger.info("")
    logger.info("")
...
def check_data_params(params):
    ...
    params.clm_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in clm_steps]   # ("de", None)
    ...
    # gmm steps # update 2020/11/12
    gmm_steps = [s.split('-') for s in params.gmm_steps.split(',') if len(s) > 0]
    params.gmm_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in gmm_steps]
    assert all([(l1 in params.langs) and (l2 in params.langs or l2 is None) for l1, l2 in params.gmm_steps])
    assert len(params.gmm_steps) == len(set(params.gmm_steps))
    ...
    # ailgn steps update 8-14
    params.align_steps = [tuple(s.split('-')) for s in params.align_steps.split(',') if len(s) > 0]   # 比如[(en,de),(en,fr)]
    assert all([len(x) == 2 for x in params.align_steps])  # 保证每一个元组都是2种语言
    assert all([l1 in params.langs and l2 in params.langs for l1, l2 in params.align_steps])   # 保证每个语言都在lgs
    assert all([l1 != l2 for l1, l2 in params.align_steps])    # 保证不相等
    assert len(params.align_steps) == len(set(params.align_steps))    # 保证没有重复的语言对

    # 新添加进来，为了保证预训练过程中的语言ID与翻译过程中的语言ID一致 update 8-11
    params.eval_mt_steps = [tuple(s.split('-')) for s in params.eval_mt_steps.split(',') if len(s) > 0]   # 比如[(en,de),(en,fr)]
    assert all([len(x) == 2 for x in params.eval_mt_steps])  # 保证每一个元组都是2种语言
    assert all([l1 in params.langs and l2 in params.langs for l1, l2 in params.eval_mt_steps])   # 保证每个语言都在lgs
    assert all([l1 != l2 for l1, l2 in params.eval_mt_steps])    # 保证不相等
    assert len(params.eval_mt_steps) == len(set(params.eval_mt_steps))    # 保证没有重复的语言对
    assert len(params.eval_mt_steps) == 0 or not params.encoder_only # Encoder only的时候，保证params.mt_steps=0
    ...
    required_mono = set([l1 for l1, l2 in (params.mlm_steps + params.clm_steps + params.gmm_steps) if l2 is None] + params.ae_steps + params.bt_src_langs)  # update 11/12 gmm steps
    params.mono_dataset = {
        lang: {
...
def load_data(params):
    ...
    # monolingual data summary
    # for splt, v in data['disc'].items():
    #    logger.info('{: <18} - {: >5}:{: >10}'.format('Discriminator data', splt, len(v['x'])))

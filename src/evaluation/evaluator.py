...
class XlmEncDecEvaluator(Evaluator):
    ...
    def __init__(self, trainer, data, params):
        """
        Build xlm_enc / encoder / decoder evaluator.
        """
        super().__init__(trainer, data, params)
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            # xlm_enc
            enc0 = xlm_enc('fwd', x=x1, lengths=len1, langs=langs1, causal=False)
            enc0 = enc0.transpose(0, 1)

            # encode source sentence
            enc1 = encoder('fwd', x=x1, xlm_output=enc0, lengths=len1, langs=langs1, causal=False)
            enc1 = enc1.transpose(0, 1)
            enc1 = enc1.half() if params.fp16 else enc1
